...
class OptoSyncWrapperE2E(unittest.TestCase):
    ...
    def test_native_adapter_boundary_stays_inside_the_installed_sdk(self) -> None:
        ...
        self.assertEqual(
            profile["repository"],
            os.environ.get("GITHUB_REPOSITORY", "anticaptrad/act-sync"),
        )
        self.assertEqual(profile["e2eRepository"], "anticaptrad/act-e2e")
    ...
    def test_terminal_state_and_provider_policy_remain_fail_closed_and_product_owned(self) -> None:
        _, _, profile = load_contract()

        retained = " ".join(profile["wrapperRetains"]).lower()
        for boundary in (
            "request/provider envelopes",
            "exact provider identity mapping",
            "fail-closed provider and terminal-state policy",
            "authorization",
            "idempotency",
            "retry",
            "lease",
        ...
        self.assertEqual(
            ...
            {"provider_jobs", "provider_observations", "reconciliation_requests"},
        )
        delegated = " ".join(profile["delegatesToOptoSync"]).lower()
        for forbidden_product_policy in (
            "provider identity",
            "terminal-state",
            "authorization",
            "lease",
            ...
            self.assertNotIn(forbidden_product_policy, delegated)
        self.assertIn("document reconciliation", delegated)
